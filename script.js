...
for (let key in achievements) {
    let saved = localStorage.getItem('ach_' + key);
    if (saved !== null) {
        achievements[key] = saved === 'true';
    }
}
...
// ========== ОБНОВЛЕНИЕ UI ==========
function updateUI() {
    if (coinCounter) coinCounter.textContent = Math.floor(coins);
    if (energyCounter) energyCounter.textContent = `${Math.floor(energy)}/${Math.floor(maxEnergy)}`;
    if (energyFill) energyFill.style.width = (energy / maxEnergy * 100) + '%';
    if (tapPowerDisplay) tapPowerDisplay.textContent = clickPower;
    if (incomeDisplay) incomeDisplay.textContent = Math.floor(passiveIncome);
    if (levelDisplay) levelDisplay.textContent = level;
    if (prestigeDisplay) prestigeDisplay.textContent = prestige;
    ...
    if (rcd) rcd.textContent = regenCost + ' 💵';
    
    const autoPrice = 5000 + autoClicker * 5000;
    let ap = document.getElementById('autoPrice');
    if (ap) ap.textContent = autoPrice + ' 💵';
    
    const offshorePrice = 5000 + offshore * 10000;
    let op = document.getElementById('offshorePrice');
    if (op) op.textContent = offshorePrice + ' 💵';
    
    const investmentPrice = 10000 + investments * 15000;
    let ip = document.getElementById('investmentPrice');
    if (ip) ip.textContent = investmentPrice + ' 💵';
...
function tap() {
    ...
    if (comboCount > 5 && comboDisplay) {
        let mult = 1 + comboCount * 0.05;
        comboDisplay.textContent = '🔥 x' + mult.toFixed(1);
        comboDisplay.style.animation = 'none';
        setTimeout(() => {
            if (comboDisplay) comboDisplay.style.animation = 'comboPulse 0.5s ease';
        }, 10);
    }
...
function claimWeeklyBonus() {
    const today = new Date().toDateString();
    if (lastWeeklyDate) {
        let last = new Date(lastWeeklyDate);
        let now = new Date();
        let diffDays = (now - last) / (1000 * 60 * 60 * 24);
        if (diffDays < 7) {
            let remaining = Math.ceil(7 - diffDays);
            showFloatingText('⏳ Бонус через ' + remaining + ' дн.');
            return;
        }
    }
...
function updateStats() {
    if (totalEarnedEl) totalEarnedEl.textContent = Math.floor(totalEarned);
    if (totalClicksDisplay) totalClicksDisplay.textContent = totalClicks;
    let bizCount = 0;
    businesses.forEach(b => bizCount += b.level);
    if (businessCountEl) businessCountEl.textContent = bizCount;
    if (prestigeStatEl) prestigeStatEl.textContent = prestige;
    
    let achieved = 0;
    for (let key in achievements) {
        if (achievements[key]) achieved++;
    }
    if (achievementCountEl) achievementCountEl.textContent = achieved + '/10';
    if (playTimeEl) playTimeEl.textContent = Math.floor(playTime / 60) + ' мин';
...
setInterval(() => {
    taxTimer--;
    if (taxTimerEl) taxTimerEl.textContent = taxTimer;
    if (taxTimer <= 0) {
        if (offshore === 0) {
            const tax = coins * 0.1;
            coins -= tax;
...
function saveGame() {
    ...
    localStorage.setItem('clickPower', clic